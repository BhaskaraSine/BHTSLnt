...
const repo = "https://api.github.com/repos/BhaskaraSine/BHTSLnt/releases/latest" // Change this if you want to change repo

export const checkVersion = () => {
    request(repo).then(response => {
        ...
        for (var i = 0; i < 3; i++) {
            if (Number(currentSplit[i]) < Number(latestSplit[i])) {
                ...
                ChatLib.chat(new Message(
                    ...
                    " ",
                    new TextComponent("&5[&dGithub&5]").setClick("open_url", "https://github.com/BhaskaraSine/BHTSLnt/releases/tag/v" + latestVersion).setHoverValue("&dClick to view the latest release on Github!"),
                    " ",
